@echo off
REM Local LLM Runtime support script for PC
REM JD-98: Implement llama.cpp-compatible runtime support with GGUF quantized models packaged via Docker & uv

set MODEL_DIR=%cd%\models
set MODEL_NAME=gpt-oss-120b-q4_k_m.gguf
set PORT=8080

echo Starting AI-Powered Job Discovery Platform Local LLM on PC via Docker...

if not exist "%MODEL_DIR%" mkdir "%MODEL_DIR%"

if not exist "%MODEL_DIR%\%MODEL_NAME%" (
    echo Model %MODEL_NAME% not found.
    echo Please download the GGUF model manually to %MODEL_DIR%\%MODEL_NAME%
)

REM Build the local LLM container (which uses uv for python deps)
docker build -t job-discovery-local-llm -f infrastructure\Dockerfile.local-llm .

if not "%OPENROUTER_API_KEY%"=="" (
    echo OPENROUTER_API_KEY is configured. Using OpenRouter integration via LiteLLM.
    docker run -d --name local-llm -p 4000:4000 -e OPENROUTER_API_KEY=%OPENROUTER_API_KEY% job-discovery-local-llm
    echo OpenRouter integration started on port 4000
) else (
    echo Starting llama-server with GPU acceleration (CUDA) and KV cache reuse...
    docker run -d --gpus all --name local-llm -p %PORT%:8080 -v "%MODEL_DIR%:/models" job-discovery-local-llm
    echo llama-server started on port %PORT%
    echo OpenAI-compatible local inference API available at http://localhost:%PORT%/v1
)
